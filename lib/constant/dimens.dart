///
const double kMainTitleAndImageHeight = 350.0;
const double kBookImageAndNameHeight = 150.0;
const double kBottomSheetHeight = 230.0;
const double kBookImageOnBottomSheetHeight = 80.0;
const double kLibraryAddIconHeight = 100.0;
const double kDialogHeight = 150.0;
const double kDialogWidth = 300.0;

///Spacing and Padding
const double kSP10x = 10.0;
const double kSP15x = 15.0;
const double kSP18x = 18.0;
const double kSP20x = 20.0;
const double kSP30x = 30.0;
const double kSP35x = 35.0;
const double kSP40x = 40.0;
const double kSP50x = 50.0;
const double kSP205x = 205.0;

///Font Size
const double kBookNameDefaultFontSize = 9.0;
const double kBookNameFontSize = 13.0;
const double kDefaultFontSize = 14.0;
const double kFontSize20x = 20.0;
const double kShelfAndCancelFontSize = 16.0;
